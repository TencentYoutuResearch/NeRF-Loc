"""
Author: jenningsliu
Date: 2022-03-10 15:35:47
LastEditors: jenningsliu
LastEditTime: 2022-07-04 20:04:47
FilePath: /nerf-loc/utils/common.py
Description: 
...
def is_inside_box3d(points, box_corners):
    ...
    :param points: (N, 3) point cloud to test in the form
    :param box_corners: (8, 3)
    ...
    # if u.P1 < u.x < u.P2
    u_dot_x = np.dot(points, u)
    u_dot_p1 = np.dot(p1, u)
    u_dot_p2 = np.dot(p2, u)

    # if v.P1 < v.x < v.P4
    v_dot_x = np.dot(points, v)
    v_dot_p1 = np.dot(p1, v)
    v_dot_p2 = np.dot(p4, v)

    # if w.P1 < w.x < w.P5
    w_dot_x = np.dot(points, w)
    w_dot_p1 = np.dot(p1, w)
    w_dot_p2 = np.dot(p5, w)
