...
class CoarseMatching(nn.Module):
    ...
    def forward(self, feat_c0, feat_c1, data, mask_c0=None, mask_c1=None):
        ...
        """
        feat_c0 = feat_c0.unsqueeze(0)
        feat_c1 = feat_c1.unsqueeze(0)
        ...
        if self.training:
            loss = self.get_matching_loss(
                data['conf_matrix_with_bin'] if self.config['sparse_spvs'] and self.match_type == 'sinkhorn' \
                    else conf_matrix, 
                data['conf_matrix_gt'].unsqueeze(0), 
                match_type=self.match_type,
                sparse_spvs=self.config['sparse_spvs'], c_pos_w=1., c_neg_w=1.)
            data.update({'coarse_loss': loss})
    ...
    def get_coarse_match(self, conf_matrix, data):
        ...
        mask = mask \
            ...
            * (conf_matrix == conf_matrix.max(dim=1, keepdim=True)[0]) # N,L,S



        # 3. find all valid coarse matches
        # this only works when at most one `True` in each row
        mask_v, all_j_ids = mask.max(dim=2) # N,L
        b_ids, i_ids = torch.where(mask_v)
        j_ids = all_j_ids[b_ids, i_ids] # N'*L'
        ...
        # mkpts0 = data['kpts0'][b_ids, i_ids]
        # mkpts1 = data['kpts1'][b_ids, j_ids]

        # # These matches is the current prediction (for visualization)
        # coarse_matches.update({
        #     'gt_mask': mconf == 0,
        #     'm_bids': b_ids[mconf != 0],  # mconf == 0 => gt matches
        #     'mkpts0': mkpts0[mconf != 0],
        #     'mkpts1': mkpts1[mconf != 0],
        #     'mconf': mconf[mconf != 0]
        # })
    ...
    def get_matching_loss(self, 
        ...
        if loss_type == 'cross_entropy':
            assert not sparse_spvs, 'Sparse Supervision for cross-entropy not implemented!'
            ...
            if sparse_spvs:
                ...
                if weight is not None:
                    ...
                    loss_neg = loss_neg * weight[neg_mask]
                return c_pos_w * loss_pos.mean() + c_neg_w * loss_neg.mean()
                # each negative element occupy a smaller propotion than positive elements.
