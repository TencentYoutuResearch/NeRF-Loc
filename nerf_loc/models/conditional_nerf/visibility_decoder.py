...
from .neuray_ops import AddBias
...
class MixtureLogisticsDistDecoder(nn.Module):
    ...
    def compute_visibility(self, depth, mean, var, vis, aw, depth_range):
        """
        Args: 
            depth: projected depth to ref views [V,N,1]
            mean: [V,N,2]
            var: [V,N,2]
            vis: [V,N,1]
            aw: [V,N,1]
    ...
    def decode_ref_depths(self, mean, depth_range):
        ...
        depth_pred_inv = mean * (far_inv - near_inv) + near_inv
        depth_pred = -1 / depth_pred_inv
        depth_pred = depth_pred.clamp(near.min(), far.max())
        return depth_pred[:,:,0] # [V,N] the first mean as ref depth
